...
export async function POST() {
  ...
  const { data: connection } = await supabase.from('integration_connections').select('id, encrypted_credentials').eq('user_id', user.id).eq('provider', 'google').maybeSingle();
  ...
  try {
    ...
    const events = (calendar.items ?? []).map((event) => ({ user_id: user.id, provider: 'google_calendar', source_id: event.id, title: event.summary || 'Untitled calendar event', starts_at: event.start?.dateTime ?? event.start?.date ?? null, ends_at: event.end?.dateTime ?? event.end?.date ?? null, is_all_day: Boolean(event.start?.date && !event.start?.dateTime), event_url: event.htmlLink ?? null, raw: event }));
    if (events.length) await supabase.from('calendar_events').upsert(events, { onConflict: 'user_id,provider,source_id' });
    ...
    const tasks = messageDetails.map((message) => ({ user_id: user.id, source: 'gmail', source_id: message.id, title: message.payload?.headers?.find((header) => header.name.toLowerCase() === 'subject')?.value || 'Unread Gmail message', kind: 'personal', priority: 'normal', notes: message.snippet ?? null }));
    if (tasks.length) await supabase.from('tasks').upsert(tasks, { onConflict: 'user_id,source,source_id' });
    await supabase.from('integration_connections').update({ status: 'connected', encrypted_credentials: encryptCredentials({ accessToken: credentials.accessToken, ...(credentials.refreshToken ? { refreshToken: credentials.refreshToken } : {}), ...(credentials.expiresAt ? { expiresAt: credentials.expiresAt } : {}) }), last_synced_at: new Date().toISOString(), error_message: null }).eq('id', connection.id);
