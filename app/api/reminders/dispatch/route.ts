...
type ReminderPreference = { user_id: string; timezone: string; digest_window_days: number | null; digest_enabled: boolean | null; digest_time: string | null };
type Digest = { days: number; timezone: string; timeline: ReturnType<typeof buildTimeline>; gmailReviews: string[] };
...
function localParts(now: Date, timezone: string) {
  const values = Object.fromEntries(new Intl.DateTimeFormat("en-CA", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(now).filter((part) => part.type !== "literal").map((part) => [part.type, part.value]));
...
export async function GET(request: Request) {
  ...
  const [{ data: reminderRows, error: reminderError }, { data: preferenceRows, error: preferenceError }] = await Promise.all([
    ...
    supabase.from("reminder_preferences").select("user_id, timezone, digest_window_days, digest_enabled, digest_time").eq("digest_enabled", true).limit(50),
  ]);
  ...
  const dueDigests = digestPreferences.map((preference) => ({ preference, ...localParts(now, preference.timezone || "UTC") })).filter(({ preference, time }) => time >= (preference.digest_time ?? "07:00").slice(0, 5));
  const delivered = dueDigests.length ? await supabase.from("email_digest_deliveries").select("user_id, digest_date").in("user_id", dueDigests.map(({ preference }) => preference.user_id)) : { data: [], error: null };
  ...
  const dailyCandidates = dueDigests.filter(({ preference, date }) => !sent.has(`${preference.user_id}:${date}`));
  const userIds = [...new Set([...reminders.map((reminder) => reminder.user_id), ...dailyCandidates.map(({ preference }) => preference.user_id)])];
  ...
  for (const message of (gmailRows ?? []) as Array<{ user_id: string; title: string }>) { const items = gmailByUser.get(message.user_id) ?? []; if (items.length < 3) items.push(message.title); gmailByUser.set(message.user_id, items); }
  ...
  const digestFor = (userId: string): Digest => { const preference = allPreferences.get(userId); const days = preference?.digest_window_days ?? 3; return { days, timezone: preference?.timezone ?? "UTC", timeline: buildTimeline(tasksByUser.get(userId) ?? [], eventsByUser.get(userId) ?? [], now, days), gmailReviews: gmailByUser.get(userId) ?? [] }; };
  const reminderJobs = reminders.map((reminder) => ({ id: reminder.id, kind: "reminder", idempotencyKey: reminder.idempotency_key, email: users.get(reminder.user_id), title: reminder.tasks?.title ?? "Upcoming task", dueAt: reminder.tasks?.due_at ?? null, digest: digestFor(reminder.user_id) }));
  const dailyJobs = dailyCandidates.map(({ preference, date }) => ({ id: `daily:${preference.user_id}:${date}`, kind: "daily_digest", idempotencyKey: `${preference.user_id}:${date}`, email: users.get(preference.user_id), title: "Your daily timeline", dueAt: null, digestUserId: preference.user_id, digestDate: date, digest: digestFor(preference.user_id) }));
  return NextResponse.json({ jobs: [...reminderJobs, ...dailyJobs].filter((job) => job.email) });
}
...
export async function POST(request: Request) {
  ...
  if (body.kind === "daily_digest") {
    if (!body.digestUserId || !/^\d{4}-\d{2}-\d{2}$/.test(body.digestDate ?? "")) return NextResponse.json({ error: "Daily digest acknowledgement is invalid." }, { status: 400 });
