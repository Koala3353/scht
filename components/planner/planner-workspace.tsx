...
export function PlannerWorkspace({ tasks: initialTasks, currentTermId, terms, subjects, projects }: PlannerWorkspaceProps) {
  ...
  return <section className="mx-auto mt-6 max-w-5xl px-4 sm:px-0">
    ...
    <div className="mt-8"><WorkManager initialProjects={projects.map((item) => ({ id: item.id, name: item.label, status: item.status }))} onTaskProjectChange={(taskId, projectId) => setTasks((current) => current.map((task) => task.id === taskId ? { ...task, projectId } : task))} tasks={tasks.filter((task) => !task.completedAt).map((task) => ({ id: task.id, title: task.title, projectId: task.projectId ?? null, dueAt: task.dueAt ?? null }))} /></div>
  </section>;
}
