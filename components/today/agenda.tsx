...
export function Agenda({ tasks, onComplete }: AgendaProps) {
  ...
  return <ol className="mt-3 space-y-3" aria-label="Current term agenda">{tasks.map((task) => {
    ...
    return <li className="grid grid-cols-[4.5rem_1fr] gap-3" key={task.id}><time className="pt-4 text-right text-sm font-bold text-slate-500" dateTime={task.dueAt ?? undefined}>{formatDueTime(task.dueAt)}</time><article className="flex min-h-24 items-start justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div><h3 className="font-bold">{task.title}</h3><p className="mt-1 text-sm text-slate-500">{task.kind[0].toUpperCase() + task.kind.slice(1)}{task.priority === 'high' ? ' · High priority' : ''}</p>{highImpact && <span className="mt-2 inline-block rounded-lg bg-orange-50 px-2 py-1 text-xs font-bold text-orange-700">HIGH IMPACT · {task.weightPercent}%</span>}</div><button type="button" aria-label={`Complete ${task.title}`} className="grid min-h-11 min-w-11 place-items-center rounded-xl text-slate-500 transition hover:bg-teal/10 hover:text-teal" onClick={() => onComplete(task)}><CheckCircle2 aria-hidden="true" /></button></article></li>;
  })}</ol>;
}
