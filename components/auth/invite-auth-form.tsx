...
export function InviteAuthForm() {
  ...
  return <div className="mt-7 space-y-4">
    <form className="space-y-3" onSubmit={submitMagicLink}>
      ...
      <input autoComplete="email" className="w-full rounded-xl border border-slate-300 bg-white px-4" id="invite-email" name="email" onChange={(event) => setEmail(event.target.value)} placeholder="you@example.edu" required type="email" value={email} />
