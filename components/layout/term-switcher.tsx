...
export function TermSwitcher({ terms, value, onChange }: TermSwitcherProps) {
  return <label className="block text-sm font-semibold text-ink">
    <span className="sr-only">Current academic term</span>
    <select aria-label="Current academic term" className="min-h-[var(--touch-target)] max-w-full rounded-xl border border-slate-300 bg-white px-3 font-medium shadow-sm" onChange={(event) => onChange(event.target.value)} value={value}>
