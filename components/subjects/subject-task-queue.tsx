...
export function SubjectTaskQueue({
  ...
  const { tasks, saveTask: saveLocalTask, retrySynchronization, syncState } = useTaskSyncWorkspace({
    userId,
    initialTasks,
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-4">
      ...
      {(saveFailure || syncState === "Sync failed") ? <div className="mt-3 rounded-xl border border-action/30 bg-[#fff8f3] p-3 text-sm text-slate-700" role="status"><p className="font-semibold text-action">{saveFailure?.reason ?? "Task sync failed. Retry this saved change."}</p><button className="mt-2 min-h-11 rounded-xl border border-action px-3 py-2 font-bold text-action" onClick={() => void retrySynchronization()} type="button">Retry saved change</button></div> : null}
