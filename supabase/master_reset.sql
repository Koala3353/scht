...
create table public.task_change_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles (id) on delete cascade,
...
create index task_change_events_user_created_idx on public.task_change_events (user_id, created_at desc);
...
begin
  if tg_op = 'INSERT' then
    event_kind := 'created';
    event_summary := case when new.source in ('canvas', 'gmail', 'google_calendar') then initcap(replace(new.source, '_', ' ')) || ' added “' || new.title || '”.' else 'Task “' || new.title || '” was created.' end;
